...
def stream_answer(prompt):
    ...
    try:
        ...
        payload = {
            ...
            "options": {
                # These parameters control response quality and style
        ...
        logger.info(f"Sending request to Ollama model: {LLM_MODEL}")
    ...
    # === ERROR HANDLING ===
        ...
        error_msg = f"Request timeout after {REQUEST_TIMEOUT} seconds. The question may be too complex or the service is overloaded."
        logger.error(error_msg)
        yield error_msg
        ...
        if e.response.status_code == 404:
            error_msg += f" - Model '{LLM_MODEL}' may not be available. Please check model name or pull the model first."
